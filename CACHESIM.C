 ...
 * way: first, write the the word including the byte to memory to update it, 
 * then bring the whole line from memory to cache and the line now in cache is
 * 'CLEAN'.
 *                                                          XuZF 2003.4.18
 */
#include	<stdio.h>
...
short	RenewLRUList( Cache * pCache, U_LONG LineNum, U_LONG PositionInCacheEntry);
U_LONG  CheckPower2( U_LONG value);


char	ErrMessage[ 250] = "Err: An error has been caught!";
...
short	CacheSim( Cache * pCache, SimResult * pSR)
{
	...
	/* Because, for example 512 is '1000000000' 10 bits in binary code.
	 * But just needs 9 bits to represent, 0 ~ 511
	 */
	BitsOfByte = GetBits( pCache->SizeOfLine) - 1;
		...
		switch (code)
		{
			...
			case 6:
				...
				if (CheckCacheFlag == -1)
				{
					...
					{
						/*
						 * After bring the line from memory to cache, and the
						 * line in cache has been marked as 'CLEAN', since it's
						 * been updated in memory before brought in cache
						 */
						CacheAdd( pCache, LineNum, tag, & TransBytes, & AccTime);
					}
				}
				else
				{
					...
					{
						/*
						 * Update the byte in cache, and mark the line as
						 * 'CLEAN', and write the word which includes the byte
						 * to the memory, and renew the LRUList
						 *
						 * Note: Don't need to worry about if the hit cache
						 * line is 'DIRTY', because since it's 'WRITE_THROUGH',
						 * not 'WRITE_BACK', such a situation will never occur.
						 * So, even we don't need do the operation that set the
						 * line's DIRTY Bit to 'CLEAN'
						 */
						/*
						(pCache->CacheTable + LineNum * pCache->associativity +
...
/*
 * FUNCTION:
 *   Brings a new memory line, which the specified mem add locates in, into the
 ...
 *   tag     - the tag of the specified memory address
 *   TransBytes - Output, transferred bytes
 *   AccTime - Output, access time
 *
